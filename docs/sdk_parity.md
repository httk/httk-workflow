...
| `Attempt.environment` | `httk_workflow_environment` | One declared workflow environment value, resolved from the job override, declared setting environment variable, workspace setting, declaration default, then the call default; an undeclared or unresolved value raises `KeyError` in Python and exits 1 in Bash. | `job.json` → `environment`, manager-built attempt environment, `context.json` → `settings` |
