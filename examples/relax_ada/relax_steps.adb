   ...
   begin
      ...
      if not Present or else U.Length (Command) = 0 then
         Status := Httk_Workflow.Httk_Workflow_Fail
           ("vasp.command_missing", "no VASP command is configured: set it with httk workflow workspace settings set vasp.command '...', or set HTTK_VASP_COMMAND, or give the job a vasp_command parameter");
         return 0;
      end if;
